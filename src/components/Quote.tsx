...
export default function Quote({ text, author, compact }: QuoteProps) {
  return (
    <section
      className={`${compact ? "py-25" : "py-30"} px-6 text-center border-t border-b border-border`}
      style={{
        background: `radial-gradient(ellipse 70% 50% at 50% 50%, rgba(200, 164, 78, 0.04) 0%, transparent 70%), var(--color-bg-warm)`,
      }}
    >
      <div className="reveal">
        <span className="font-serif text-[80px] text-gold-dim leading-[0.5] mb-6 block">
          &ldquo;
        </span>
